...
class OmDeterminations extends Manager{
	...
	public function __construct($conn){
		...
		$this->schemaMap = array('identifiedBy' => 's', 'dateIdentified' => 's', 'higherClassification' => 's', 'family' => 's', 'sciname' => 's', 'verbatimIdentification' => 's',
			...
			'identificationVerificationStatus' => 's', 'taxonConceptID' => 's', 'sourceIdentifier' => 's', 'sortSequence' => 'i', 'createdUid' => 'i', 'tidInterpreted' => 'i', 'modifiedUid' => 'i');
	}
		...
		foreach($this->schemaMap as $field => $type){
			...
			if($postField){
				...
				if($value){
					...
					elseif($postField == 'createduid') $value = OccurrenceUtilities::verifyUser($value, $this->conn);
					elseif($type == 'i'){
						if(!is_numeric($value)){
